...
export const academyRegistrations = pgTable('academy_registrations', {
  ...
  batch:             text('batch'),
  // Profession-conditional detail (per registration-form spec). Only the fields
  // relevant to the chosen profession are ever populated; the rest stay null.
  ...
  otherProfessionDetail:  text('other_profession_detail'),
...
export const academyInterestRegistrations = pgTable('academy_interest_registrations', {
  ...
  location:       text('location'),
  country:        text('country'),
  // Which org department this enquiry is for (HR, Sales, Finance, ...) — set when the
  // enquiry is a request for department-customised training, null otherwise.
  department:            text('department'),
  courseName:            text('course_name'),
  currentYear:           text('current_year'),
  subjectSpecialization: text('subject_specialization'),
